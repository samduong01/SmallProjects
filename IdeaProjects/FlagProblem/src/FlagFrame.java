...
public class FlagFrame extends JFrame {
    ...
    private void clearWindow(Graphics g, Graphics2D gr){
    ...
    private void updateScale(){
        ...
        this.scale = this.getHeight()-offset;
    ...
    private void drawBounds(Graphics g){
        ...
        g.drawRect(originCords[0], originCords[1],b,(int)(l*13.0));
    }

    private void drawStars(Graphics2D gr){
        int incrementY = originCords[1]+e;
        ...
        for(int j = 0;j<9;j++) {
            ...
            //else draw 5 stars
            if(j%2 == 0) {
                int x = g+ originCords[0];
                for (int i = 0; i < 6; i++) {
                    gr.fill(createStar(x, incrementY, (k/5.0), k/2.0));
                    x += 2 * h;
                }
            }else{
                int x = g+h+ originCords[0];
                for(int i = 0;i<5;i++){
                    gr.fill(createStar(x, incrementY, (k/5.0), k/2.0));
                    x+=2*h;
                }
